                                ...
                                {visibleComps.length > 0 ? visibleComps.map(comp => (
                                    <div key={comp.id} className="p-4 hover:bg-slate-50/50 transition-colors group relative">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                            ...
                                            {/* Cambio visual y de key para Presión */}
                                            <div className="space-y-1">
                                                <label className="text-xs font-semibold text-slate-500 flex items-center"><Gauge size={12} className="mr-1" /> Initial Pressure</label>
                                                <input type="number" step="0.01" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-slate-900 text-white focus:ring-1 focus:ring-blue-500 outline-none" placeholder="0.00" value={readings[comp.id]?.initial_pressure || ''} onChange={(e) => handleInputChange(comp.id, 'initial_pressure', e.target.value)} required />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-xs font-semibold text-slate-500 flex items-center"><Gauge size={12} className="mr-1" /> Final Pressure</label>
                                                <input type="number" step="0.01" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-slate-900 text-white focus:ring-1 focus:ring-blue-500 outline-none" placeholder="0.00" value={readings[comp.id]?.final_pressure || ''} onChange={(e) => handleInputChange(comp.id, 'final_pressure', e.target.value)} required />
                                            </div>
                                        </div>
                                    </div>
