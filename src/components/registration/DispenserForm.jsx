...
export default function DispenserForm({ onCancel, onSave, isNested = false, initialData = null }) {
  ...
  const [hoseCount, setHoseCount] = useState(initialData?.hoses?.length || '');
  const [isAddingTag, setIsAddingTag] = useState(false);
  const [currentTag, setCurrentTag] = useState('');
  const [addingTagForHose, setAddingTagForHose] = useState(null);
  ...
  const handleDispenserTagKeyDown = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      handleAddDispenserTag();
    }
  };

  const handleAddHoseTag = (index) => {
    const text = hoseTagInputs[index];
    if (text && text.trim()) {
      const updatedHoses = [...formData.hoses];
      const currentTags = updatedHoses[index].tags || [];

      if (!currentTags.includes(text.trim())) {
        updatedHoses[index].tags = [...currentTags, text.trim()];
        setFormData(prev => ({ ...prev, hoses: updatedHoses }));
      }
      setHoseTagInputs(prev => ({ ...prev, [index]: '' }));
    }
    setAddingTagForHose(null);
  };
  ...
  const handleHoseTagKeyDown = (e, index) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      handleAddHoseTag(index);
    }
  };
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2">
      <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        <h3 className="text-lg font-bold text-slate-800">
          {isNested ? 'Add Dispenser Configuration' : initialData ? 'Edit Dispenser Details' : 'New Dispenser Registration'}
        </h3>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">Island</label>
              <input
                name="island"
        ...
        <div>
          <label className="text-sm font-medium text-slate-700 mb-2 block">Dispenser Tags / Labels</label>
          <div className="flex flex-wrap gap-2 items-center p-3 bg-slate-50 rounded-lg border border-slate-200">
            {formData.tags.map((tag, idx) => (
              <span key={idx} className="flex items-center bg-white border border-slate-300 text-slate-700 px-3 py-1 rounded-full text-sm shadow-sm">
                ...
                {tag}
                <button type="button" onClick={() => removeDispenserTag(tag)} className="ml-2 text-slate-400 hover:text-red-500"><X size={14} /></button>
              </span>
            ))}
            ...
            {!isAddingTag ? (
              ...
              <div className="flex items-center animate-in fade-in slide-in-from-left-2">
                <input
                  ...
                  onKeyDown={handleDispenserTagKeyDown}
                  onBlur={() => { if (!currentTag) setIsAddingTag(false); }}
                  className="px-3 py-1 border border-blue-500 rounded-l-full focus:outline-none text-sm w-32"
                  placeholder="Type tag..."
                />
                <button
                  type="button"
                  onClick={handleAddDispenserTag}
              ...
              <div key={index} className="flex flex-col md:flex-row gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200 animate-in slide-in-from-left-2">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2 mt-2 border-t border-slate-200 pt-3">
                    <label className="text-xs font-semibold text-slate-500 mb-2 block">Hose Tags</label>
                    <div className="flex flex-wrap gap-2 items-center">
                      {hose.tags && hose.tags.map((tag, tIdx) => (
                        <span key={tIdx} className="flex items-center bg-white border border-slate-200 text-slate-600 px-2 py-1 rounded text-xs shadow-sm">
                          {tag}
                          <button type="button" onClick={() => removeHoseTag(index, tag)} className="ml-1 text-slate-400 hover:text-red-500"><X size={12} /></button>
                        </span>
                      ))}

                      {addingTagForHose !== index ? (
                        <button
                          type="button"
                          onClick={() => setAddingTagForHose(index)}
                          className="flex items-center space-x-1 px-2 py-1 rounded border border-dashed border-slate-400 text-slate-500 hover:bg-white hover:text-blue-600 hover:border-blue-500 text-xs transition-colors"
                        >
                          <Plus size={12} />
                          <span>Add Tag</span>
                        </button>
                        ...
                        <div className="flex items-center animate-in fade-in slide-in-from-left-2">
                          <input
                            ...
                            onKeyDown={(e) => handleHoseTagKeyDown(e, index)}
                            onBlur={() => { if (!hoseTagInputs[index]) setAddingTagForHose(null); }}
                            className="px-2 py-1 border border-blue-500 rounded-l focus:outline-none text-xs w-24"
                            placeholder="Tag..."
                          />
                          <button
                            type="button"
                            onClick={() => handleAddHoseTag(index)}
