...
export default function TankList() {
  ...
  const getFuelInfo = (id) => {
    const fuels = {
      1: { name: 'Gasolina Corriente', color: 'bg-yellow-100 text-yellow-700' },
      2: { name: 'Gasolina Extra', color: 'bg-red-100 text-red-700' },
      3: { name: 'Diesel', color: 'bg-slate-200 text-slate-700' }
    };
    return fuels[id] || { name: 'Unknown', color: 'bg-gray-100 text-gray-500' };
  };

  const handleSave = (newTankPayload) => {
    console.log('[TankList] Received new tank payload:', newTankPayload);
  ...
  if (isCreating) {
    return <TankForm onCancel={() => setIsCreating(false)} onSave={handleSave} />;
  }
        ...
        <div className="relative">
            ...
            <input
                ...
                onChange={(e) => {
                    ...
                    setIsSearchOpen(true);
                    // IMPORTANTE: No borramos selectedStation aquí para mantener la tabla visible mientras editas
                    ...
                    <div key={tank.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex flex-col hover:border-blue-300 transition-colors">
                        ...
                        {tank.compartments.map((comp, idx) => {
                            ...
                            <div key={idx} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-slate-700">Code: {comp.code}</p>
